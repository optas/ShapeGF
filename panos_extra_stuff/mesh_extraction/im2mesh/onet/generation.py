...
from panos_extra_stuff.mesh_extraction.im2mesh.utils.libmise import MISE
from panos_extra_stuff.mesh_extraction.im2mesh.utils import libmcubes
import time
...
class Generator3D(object):
    ...
    def generate_from_latent(self, z, sigma=None, stats_dict={}, **kwargs):
        ...
        if self.upsampling_steps == 0:
            ...
            values = self.model.eval_points(pointsf, z, sigma, **kwargs).cpu().numpy()
            ...
            while points.shape[0] != 0:
                ...
                values = self.model.eval_points(
                    pointsf, z, sigma, **kwargs).cpu().numpy()
        ...
        mesh = self.extract_mesh(value_grid, z, sigma, stats_dict=stats_dict)
        return mesh
    ...
    def eval_points(self, p, z, sigma=None, **kwargs):
        ...
        for pi in p_split:
            ...
            with torch.no_grad():
                grad = self.model.score_net(pi, z_sigma, opt='field')

            grad.append(grad.squeeze(0).detach().cpu())
